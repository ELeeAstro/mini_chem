program mini_chem_main
  ...
  integrator = 'radau5'
  open(newunit=u,file='outputs_radau5/'//trim(integrator)//'.txt',status='replace',action='write')
  ...
  do n = 1, n_step
    ...
    ! - don't send He to integrator so dimensions are n_sp-1
    call mini_ch_radau5(T_in, P_in, t_step, VMR(1:n_sp-1), network)
