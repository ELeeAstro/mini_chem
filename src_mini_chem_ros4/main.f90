program mini_chem_main
  ...
  use mini_ch_i_ros4, only : mini_ch_ros4
  implicit none
  ...
  do n = 1, n_step
    ...
    ! Call ros4 - Rosenbrock method - don't send He to integrator so dimensions are n_sp-1
    call mini_ch_ros4(T_in, P_in, t_step, VMR(1:n_sp-1), network)
    print*, 'ros4: ', VMR(:), sum(VMR(:))
    write(u,*) n, t_now, VMR(:)
