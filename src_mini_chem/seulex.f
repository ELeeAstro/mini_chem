...
C -------- CALL TO CORE INTEGRATOR ------------
      CALL SEUCOR(N,FCN,X,Y,XEND,HMAX,H,KM,RTOL,ATOL,ITOL,JAC,IJAC,
     &   MLJAC,MUJAC,MAS,MLMAS,MUMAS,SOLOUT,IOUT,IDID,IJOB,M1,M2,NM1,
     ...
     &   RPAR,IPAR,WORK(IEDE),IWORK(IECO))
      IWORK(14)=NFCN
      IWORK(15)=NJAC
      IWORK(16)=NSTEP
      IWORK(17)=NACCPT
...
C
      SUBROUTINE SEUCOR(N,FCN,X,Y,XEND,HMAX,H,KM,RTOL,ATOL,ITOL,
     &  JAC,IJAC,MLJAC,MUJAC,MAS,MLB,MUB,SOLOUT,IOUT,IDID,IJOB,M1,M2,
     ...
     &  NDEC,NSOL,RPAR,IPAR,DENS,ICOMP)
C ----------------------------------------------------------
...
C ----------------------------------------------------------
       ...
       DIMENSION WH(N),SCAL(N),HH(KM),W(KM),A(KM),FJAC(LFJAC,N)
       DIMENSION FMAS(LDMAS,NM1),T(KM,N),IP(NM1),NJ(KM),RTOL(*),ATOL(*)
       DIMENSION IPHES(N),FSAFE(KM2,NRD),FACUL(KM),E(LE,NM1)
       DIMENSION DENS((KM+2)*NRD),ICOMP(NRD),RPAR(*),IPAR(*)
...
c$omp threadprivate(/COSEU/,/LINAL/)
...
C --- DEFINE THE STEP SIZE SEQUENCE
       ...
       IF (IOUT.NE.0) THEN
          ...
          NSOLU=N
          XOLD=X
         CALL SOLOUT(NRSOL,XOLD,XSOL,YH,DENS,LRDE,ICOMP,NRD,NSOLU,
     &               RPAR,IPAR,IRTRN)
...
C --- COMPUTE JACOBIAN MATRIX NUMERICALLY
              IF (.NOT.AUTNMS) CALL FCN(N,X,Y,DY,RPAR,IPAR)
              IF (BANDED) THEN
...
C --- COMPUTE JACOBIAN MATRIX ANALYTICALLY
               CALL JAC(N,X,Y,FJAC,LFJAC,RPAR,IPAR)
           END IF
           CALJAC=.TRUE.
           CALHES=.TRUE.
       END IF
...
C
     ...
     &          IPT,M1,M2,NM1,IJOB,CALHES,RPAR,IPAR)
...
C --- OF THE OPTIMAL STEP SIZE
      ...
      DIMENSION FSAFE(KM2,NRD),ICOMP(NRD),RPAR(*),IPAR(*)
...
C *** *** *** *** *** *** ***
     ...
     &            M1,M2,NM1,HJI,E,LE,IP,IER,IJOB,CALHES,IPHES)
      IF (IER.NE.0) GOTO 79
      NDEC=NDEC+1
C *** *** *** *** *** *** ***
...
C *** *** *** *** *** *** ***
       IF (.NOT.AUTNMS) THEN
          CALL FCN(N,X+HJ,Y,DY,RPAR,IPAR)
          NFCN=NFCN+1
       END IF
       DO I=1,N
          YH(I)=Y(I)
...
C *** *** *** *** *** *** ***
       IF (M.GT.1) THEN
          DO MM=1,M-1
             ...
             END DO
             IF (AUTNMS) THEN
                CALL FCN(N,X+HJ*MM,YH,DYH,RPAR,IPAR)
             ELSE
                CALL FCN(N,X+HJ*(MM+1),YH,DYH,RPAR,IPAR)
             END IF
             NFCN=NFCN+1
...
C --- STABILITY CHECK
                ...
                END IF
                IF (.NOT.AUTNMS) THEN
                   CALL FCN(N,X+HJ,YH,WH,RPAR,IPAR)
                   NFCN=NFCN+1
                   DO I=1,N
...
C --- COMPUTE OPTIMAL STEP SIZES
  ...
  79   ATOV=.TRUE.
       ...
       END
C
      FUNCTION CONTEX(II,X,RC,LRC,IC,LIC)
C ----------------------------------------------------------
C     THIS FUNCTION CAN BE USED FOR CONINUOUS OUTPUT IN CONECTION
C     WITH THE OUTPUT-SUBROUTINE FOR SEULEX. IT PROVIDES AN
C     APPROXIMATION TO THE II-TH COMPONENT OF THE SOLUTION AT X.
C ----------------------------------------------------------
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION RC(LRC),IC(LIC)
      COMMON /COSEU/XOLD,H,NRD,IR
c$omp threadprivate(/COSEU/)
