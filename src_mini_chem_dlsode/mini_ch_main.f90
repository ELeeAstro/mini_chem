program mini_chem_main
  ...
  integrator = (/'dlsode     '/)
  do n = 1, n_solver
    open(newunit=u(n),file='outputs_dlsode/'//trim(integrator(n))//'.txt',action='readwrite')
  ...
  do n = 1, n_step
    ...
    call mini_ch_dlsode(T_in, P_in, t_step, VMR_cp(1,:), network)
    print*, 'dlsode: ', VMR_cp(1,:), sum(VMR_cp(1,:))
