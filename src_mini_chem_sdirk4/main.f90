program mini_chem_main
  ...
  use mini_ch_i_sdirk4, only : mini_ch_sdirk4
  implicit none
  ...
  integrator = 'ros4'
  open(newunit=u,file='outputs_sdirk4/'//trim(integrator)//'.txt',status='replace',action='write')
  ...
  do n = 1, n_step
    ...
    ! Call sdirk4 - singly-diagonal implicit Runge-Kutta method - don't send He to integrator so dimensions are n_sp-1
    call mini_ch_sdirk4(T_in, P_in, t_step, VMR(1:n_sp-1), network)
    print*, 'sdirk4: ', VMR(:), sum(VMR(:))
    write(u,*) n, t_now, VMR(:)
