! Made into .f90 standards and optimised - Graham Lee - May 2016
...
!c-----------------------------------------------------------------------
        ...
        do j1 = j, k
          do j2 = j1, j, - 1
            ...
            do i = 1, n
              Work(i,j2) = Work(i,j2+1) + tmp * (Work(i,j2+1)-Work(i,j2))
            end do
          end do
        end do
        ...
        if (j < k) then
          do j1 = j, k
            do i = ipt(j1+1), ipt(j1+1) - j1 + j, - 1
              call daxpy(n,-one,Dense(1,i-1),1,Dense(1,i),1)
            end do
          end do
        end if
...
!c-----------------------------------------------------------------------
    ...
    end subroutine Eval_Herm


    subroutine JacPlot(n,LenJac,Jac,LDJac,ml,mu,ir,ic,nStep,mode)
      implicit none
...
! c-----------------------------------------------------------------------
! c
      integer :: n,LenJac,LDJac,ml,mu,ir(*),ic(*),nStep,mode
...
!c-----------------------------------------------------------------------
      ...
      real :: Bottom,cm_dot,Left,Margin,Right,Scaling,Top,xTitle,yTitle
...
!c
!c-----------------------------------------------------------------------
      real, parameter :: PaperSize = 21.0, PlotSize = 15.0
...
!c-----------------------------------------------------------------------
      ...
      write ( 99, 9000 ) '/Cshow {dup stringwidth pop -2 div 0 rmoveto show} def'
      write ( 99, 9000 ) 'gsave'
      write ( 99, 9000 ) '/Helvetica findfont 0.5 cm scalefont setfont'
      write ( 99, 9020 ) xTitle, ' cm ', yTitle, ' cm moveto'
      write ( 99, 9030 ) '(Jacobian matrix from LIMEX4.2, step = ',nStep, ') Cshow'
...
!c-----------------------------------------------------------------------
      ...
      if ( mode == 0 ) then
        do i = 1, n
          do j = 1, n
            if ( Jac(j,i) /= 0.0d0 ) then
              write ( 99, 9060 ) i - 1, n - j, 'p'
            end if
          end do
        end do
