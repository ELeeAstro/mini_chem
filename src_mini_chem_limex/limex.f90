...
!c
      ...
      double precision,intent(inout) :: y(n), ys(n)
      double precision,intent(in)    :: rTol(*), aTol(*)
      double precision,intent(inout) :: h
      integer,intent(inout)          :: Iopt(30)
      double precision,intent(in)    :: Ropt(5)
      integer,intent(in)             :: IPos(n)
      integer,intent(out)            :: IFail(3)
! c
! c-----------------------------------------------------------------------
! c
...
!c
     ...
     &                   Max_Non_Zeros_B, &
     &                   Max_Lower_Diagonals, Max_Upper_Diagonals, &
     &                   Max_It_Vectors
!c
!c-----------------------------------------------------------------------
!c
...
!c-----------------------------------------------------------------------
!c
      integer, parameter :: Max_Size = Max_Nr_of_Equations
! c
! c-----------------------------------------------------------------------
! c
...
! c                        values
! c
      integer, parameter :: Max_Int_Steps = 1000
      integer, parameter :: Max_Step_Red_Ex = 50
      integer, parameter :: Max_Step_Red_Dc = 30
      integer, parameter :: Max_Row_Tab = 30
      integer, parameter :: Max_Step_CIV = 30
...
!c-----------------------------------------------------------------------
!c
      logical,save ::    CIV_Convergence, Convergence, Direct
!c
      integer,save ::    DensOut, FcnInfo, Full_or_Band, HoldJac, i, &
     &                   icc, iCIV, iCont, iLower, iRed, irh, irr, &
     &                   iSing, Iter_Rich, iUpper, j, j_Cons, JacAct, &
     &                   JacInfo, JacP, jCIV, jk, jo, joh, jmax, jmh, &
     &                   jmm, k, k_Cons, k_Dense, k1, kc1, kc2, kl, km, &
     &                   ko, koh, mb1, mb2, ml, MonOut, mu, n_Dense, &
     &                   nDec, NewJac, nFcn, nFcnE, nFcnJ, nJac, njj, &
     &                   NoDense, NonPosComp, nSol, nStep, nz, SolOut, &
     &                   Status
!c
      double precision,save ::   abs_del, CErr, Cons_Err, d1, d2, delm, &
     &                   delta_Dense, delta_y, Delta_y0,delta_y_rec, &
     &                   del_quot, eph, eps_Mach, eps_Mach10, EtaDif, &
     &                   fc, fcm, fco, fn, g, g_inv, h_proposed, h_True, &
     &                   hMax, hMax_Int, hSave, Mean_rTol, omj, omjo, &
     &                   red, rTol2, t, t_dense, tEnd_t, th, tmp, &
     &                   tn_Dense, TolQuot, WarnCont, y_orig
!c
...
!c-----------------------------------------------------------------------
!c
      double precision, dimension(Max_Size), save :: del,dz,Scal, &
        & Temp1, Temp2, Temp3, Temp4, TolQu, yk
! c
! c-----------------------------------------------------------------------
! c
...
! c-----------------------------------------------------------------------
! c
      integer, dimension(Max_Row_Tab), save :: nj, incr
!c
      double precision, dimension(Max_Row_Tab), save :: aj
!c
      double precision, dimension(Max_Row_Tab,Max_Row_Tab), save :: al, d
!c
      double precision, dimension(Max_Size,Max_Row_Tab), save ::  dt, dtp
! c
! c-----------------------------------------------------------------------
! c
...
! c-----------------------------------------------------------------------
! c
      integer, dimension(Max_Non_Zeros_B), save :: ic, ir
!c
      double precision, dimension(Max_Non_Zeros_B), save :: b0, bk
! c
! c-----------------------------------------------------------------------
! c
...
! c-----------------------------------------------------------------------
! c
      integer, dimension(Max_Row_Tab), save :: ipt
!c
      double precision, &
      & dimension(Max_Size,2+(Max_Row_Tab*(Max_Row_Tab+1))/2), save :: Dense
! c
! c-----------------------------------------------------------------------
! c
...
! c-----------------------------------------------------------------------
! c
      integer, dimension(Max_Size), save :: Pivot
!c
  integer,parameter :: Max_Band_Size_1=1+Max_Lower_Diagonals+Max_Upper_Diagonals
  integer,parameter :: Max_Band_Size_2 = Max_Band_Size_1 + Max_Lower_Diagonals
  integer,parameter :: mbs1 = Max_Band_Size_2 / Max_Size
  integer,parameter :: mbs2 = Max_Size / Max_Band_Size_2
  integer,parameter :: mbs3 = ( mbs1 + 2 ) / ( mbs1 + 1 ) - 1
  integer,parameter :: mbs4 = ( mbs2 + 2 ) / ( mbs2 + 1 ) - 1
  integer,parameter :: LDJac  =  mbs3 * Max_Band_Size_1 + mbs4 * Max_Size
  integer,parameter :: LDBJac = mbs3 * Max_Band_Size_2 + mbs4 * Max_Size

      double precision, save :: Jac(LDJac,Max_Size), B_Jac(LDBJac,Max_Size)
! c
! c-----------------------------------------------------------------------
! c
...
! c-----------------------------------------------------------------------
! c
      integer, parameter :: iCons1 = 4, iCons2 = 4, Max_Iter_Rich = 10
!c
      double precision, parameter :: CStep = 1.0d-7, Epmin = 1.0d-10, ex1 = 0.6d0, &
        & ex2 = 1.5d0, fmin = 1.0d-2, h_Min = 1.0d-4, &
        & one = 1.0d0, one1 = 1.01d0, quart = 0.25d0, &
        & RedFcn = 0.5d0, RedPos = 0.5d0, rmin = 0.9d0, &
        & ro = 0.25d0, safe = 0.5d0, Small = 1.0d-30, &
        & ten = 10.0d0, ThMin = 2.0d-2, thresh = 0.1d0, &
        & zero = 0.0d0
! c
! c
! c-----------------------------------------------------------------------
...
!$omp threadprivate( CIV_Convergence, Convergence, Direct )
!$omp threadprivate( DensOut, FcnInfo, Full_or_Band, HoldJac, i )
!$omp threadprivate( icc, iCIV, iCont, iLower, iRed, irh, irr )
!$omp threadprivate( iSing, Iter_Rich, iUpper, j, j_Cons, JacAct )
!$omp threadprivate( JacInfo, JacP, jCIV, jk, jo, joh, jmax, jmh )
!$omp threadprivate( jmm, k, k_Cons, k_Dense, k1, kc1, kc2, kl, km )
!$omp threadprivate( ko, koh, mb1, mb2, ml, MonOut, mu, n_Dense )
!$omp threadprivate( nDec, NewJac, nFcn, nFcnE, nFcnJ, nJac, njj )
!$omp threadprivate( NoDense, NonPosComp, nSol, nStep, nz, SolOut )
!$omp threadprivate( Status )
!$omp threadprivate( abs_del, CErr, Cons_Err, d1, d2, delm )
!$omp threadprivate( delta_Dense, delta_y, Delta_y0,delta_y_rec )
!$omp threadprivate( del_quot, eph, eps_Mach, eps_Mach10, EtaDif )
!$omp threadprivate( fc, fcm, fco, fn, g, g_inv, h_proposed, h_True )
!$omp threadprivate( hMax, hMax_Int, hSave, Mean_rTol, omj, omjo )
!$omp threadprivate( red, rTol2, t, t_dense, tEnd_t, th, tmp )
!$omp threadprivate( tn_Dense, TolQuot, WarnCont, y_orig )
!$omp threadprivate( del )
!$omp threadprivate( dz )
!$omp threadprivate( Scal )
!$omp threadprivate( Temp1 )
!$omp threadprivate( Temp2 )
!$omp threadprivate( Temp3 )
!$omp threadprivate( Temp4 )
!$omp threadprivate( TolQu )
!$omp threadprivate( yk    )
!$omp threadprivate( nj    )
!$omp threadprivate( incr  )
!$omp threadprivate( aj    )
!$omp threadprivate( al    )
!$omp threadprivate( d     )
!$omp threadprivate( dt    )
!$omp threadprivate( dtp   )
!$omp threadprivate( ic    )
!$omp threadprivate( ir    )
!$omp threadprivate( b0    )
!$omp threadprivate( bk    )
!$omp threadprivate( ipt   )
!$omp threadprivate( Dense )
!$omp threadprivate( Pivot )
!$omp threadprivate( Jac   )
!$omp threadprivate( B_Jac )
...
!c
      if ( Max_Nr_of_Equations .lt. n .or. n .lt. 1 ) then
         if ( Iopt(1) .ne. 0 ) write ( MonOut, 9040 ) n, Max_Nr_of_Equations
         IFail(1) = - 1
         go to 510
      end if
!c
...
!c
      if ( Iopt(13) .eq. 0 .or. ( Iopt(13) .eq. 1 .and. Iopt(14) .le. 1 ) &
        &  .or. ( Iopt(13) .eq. 2 .and. Iopt(14) .eq. 0 ) ) then
         DensOut = 0
         NoDense = 0
      else
         NoDense = 1
...
!c       Is the next line a bug? Graham Lee (May 2016)
         if ( Iopt(7) .eq. 0 ) then
            write ( MonOut, 9250 ) Iopt(8), Iopt(9)
         end if
!c
...
!c
         do i = 2, j-1
            al(j-1,i-1) = eph **((aj(i)-aj(j)) / ((aj(j)-aj(1))*dble(i)))
         end do
!c
      end do
!c
...
!c
      ...
      & .and. ( Iopt(3) .lt. 2 .or. DensOut .ne. SolOut ) ) &
      & write ( DensOut, 9480 ) t_Begin, ( y(i), i = 1, n )
!c
      if ( Iopt(1) .gt. 0 ) write ( MonOut, 9490 )
!c
      if ( abs ( t_End - t ) .lt. abs ( t ) * eps_Mach10 * ten &
     &     .or. abs ( t_End - t ) .lt. abs ( h ) * eps_Mach10 ) &
     &   go to 360
!c
!c-----------------------------------------------------------------------
!c
...
!c
                     if ( yk(i) .ne. del(i) ) then
                        Jac(i,j) = (yk(i) - del(i)) * (delta_y_rec / Scal(i))
                     else
                        Jac(i,j) = zero
                     end if
!c
...
!c
                        if ( yk(i) .ne. del(i) ) then
                           Jac(k1+i,k) = (yk(i) - del(i)) * (delta_y_rec / Scal(i))
                        else
                           Jac(k1+i,k) = zero
                        end if
!c
...
!c
            call Jacobian(n,t,y,ys,Jac,LDJac,ml,mu,Full_or_Band,JacInfo)
!c
...
!c
     ...
     &        .or. ( JacP .eq. nStep .and. iCIV .eq. 0 &
     &                               .and. nStep .gt. 0 ) ) then
!c
...
!c
      n_Dense = 0
!c
      if ( iCIV .eq. 0 .and. Iopt(13) .eq. 1 .and. Iopt(14) .gt. 2 ) then
!c
         do i = 0, Iopt(14) - 3
!c
...
!c
     ...
     &     .and. (      ( Iopt(13) .eq. 2 .and. Iopt(14) .gt. 0 ) &
     &             .or. ( Iopt(13) .eq. 3 .and. h .gt. Ropt(2) ) &
     &             .or. ( Iopt(17) .eq. 1 .and. t_End .le. t + h ) ) ) &
     &   n_Dense = 1
!c
  160 continue
!c
...
!c
            B_Jac(irh,icc) = B_Jac(irh,icc) + Scal(icc) * b0(i) / (g*Scal(irr))
!c
         end do
!c
!c-----------------------------------------------------------------------
...
!c
     ...
     &        .and. j .le. 2    .and. nStep .le. 2 ) then
!c
            if ( j .eq. 1 ) then
!c
               delm = zero
...
!c        iopt(14) = 100         ! number of equidistant dense output points
...
!c
                  Temp3(irr) = Temp3(irr) &
     &              + Scal(icc) * bk(i) * Temp2(icc) / ( g * Scal(irr) )
!c
               end do
!c
            end if
...
!c
            if ( n_Dense .eq. 1 ) &
     &         call dcopy ( n, Temp3, 1, Dense(1,ipt(j-1)+k+1), 1 )
!c
!c-----------------------------------------------------------------------
!c
...
!c
      if ( Iopt(1) .eq. 2 ) &
     &   write ( MonOut, 9560 ) iRed, NonPosComp, RedPos
!
      if ( iRed .gt. Max_Step_Red_Ex ) go to 450
!
      go to 290
...
!-----------------------------------------------------------------------
!
!     Empirical stepsize reduction due to an internal error in Fcn.
!
!-----------------------------------------------------------------------
!
  280 continue
...
!
      if ( Iopt(1) .eq. 2 ) &
     &   write ( MonOut, 9570 ) iRed, FcnInfo, RedFcn
!
      if ( iRed .gt. Max_Step_Red_Ex ) go to 450
!c
!c-----------------------------------------------------------------------
...
!c
         if (       t_Dense .lt. t + h &
     &        .and. t_End - t_Dense .gt. abs ( t ) * eps_Mach10 ) &
     &      then
!c
...
!c
         if ( DensOut .ne. 0 ) &
     &      write ( DensOut, 9480 ) t_Dense, ( Temp1(j), j = 1, n )
!c
         if ( Iopt(12) .eq. 2 ) then
!c
             call dcopy ( n, Temp1, 1, y, 1 )
...
!c
         if (        DensOut .ne. 0 &
     &        .and. ( Iopt(13) .eq. 1 .or. Iopt(13) .eq. 3 ) ) &
     &      write ( DensOut, 9480 ) t_End, ( Temp1(j), j = 1, n )
!c
...
!c
         if ( Iopt(3) .gt. 0 ) &
     &      write ( SolOut, 9510 ) t_End, ( y(i), i = 1, n )
!c
...
!c
      if (       (        Iopt(13) .gt. 1 &
     &             .or. ( Iopt(13) .eq. 1 .and. nStep .eq. 0 ) ) &
     &     .and. DensOut .ne. 0 &
     &     .and. ( Iopt(3) .eq. 0 .or. SolOut .ne. DensOut ) ) &
     &   write ( DensOut, 9480 ) t, ( y(i), i = 1, n )
!c
      if ( Iopt(17) .ne. 1 ) then
!c
         if (      abs( tEnd_t ) .lt. abs( t ) * eps_Mach10 * ten &
     &        .or. abs( tEnd_t ) .lt. abs( h ) * eps_Mach10 ) &
     &      go to 360
!c
      else if ( iCont .eq. 1 ) then
!c
         if (      abs( Ropt(3) - t ) .lt. abs( t ) * eps_Mach10 * ten &
     &        .or. abs( Ropt(3) - t ) .lt. abs( h ) * eps_Mach10 ) &
     &      go to 360
!c
      end if
      ...
      if (         jCIV .eq. 0 &
     &     .and. (        Iopt(12) .eq. 1 &
     &             .or. (       Iopt(12) .eq. 2 .and. NoDense .eq. 1 &
     &                    .and. Iopt(13) .ge. 2 ) ) ) then
!c
         Iopt(24) = nFcnE
         Iopt(25) = nFcnJ
         Iopt(26) = nDec
...
!c
     ...
     &     .and. ( Iopt(3) .eq. 0 .or. SolOut .ne. DensOut ) ) &
     &   write ( DensOut, 9480 ) t_End, ( y(i), i = 1, n )
!c
      if ( Iopt(3) .gt. 0 ) &
     &   write ( SolOut, 9510 ) t_End, ( y(i), i = 1, n )
!c
...
!c
      if ( Iopt(1) .gt. 0 ) &
     &   write ( MonOut, 9610 ) nz, Max_Non_Zeros_B
!c
      IFail(1) = - 34
!c
      go to 500
...
!c
 9000 format ( /, 16x, 43('-'), /, 16x, '|', 41x, '|', /, 16x, '|', 11x, &
     &         'L I M E X  V. 4.2A1', 11x, '|', /, 16x, '|', 41x, '|', &
     &         /, 16x, '| Copyright (C) 2000, Konrad-Zuse-Zentrum |', /, &
     &         16x, '|  fuer Informationstechnik Berlin (ZIB)  |', /, &
     &         16x, 43('-'), / )
 ...
 9040 format ( ' *** LIMEX error, wrong input for n : ', i5, ',', 23x, &
     &         '***', /, ' *** should be between 1 and ', i5, 33x, &
     &         '***' )
 ...
 9080 format ( ' *** LIMEX error, wrong input for rTol(', i5, ') :', &
     &         d18.9, ', ***', /, ' *** should be greater 0.0d0', 39x, &
     &         '***' )
 ...
 9110 format ( 9x, 57('-'), /, 9x, '|', 55x, '|', /, 9x, &
     &         '| Integration options:', 34x, '|', /, 9x, '|', 55x, &
     &         '|' )
 ...
 9250 format ( 9x, '| Lower bandwidth of the Jacobian:', i5, 17x, '|', &
     &         /, 9x, '| Upper bandwidth of the Jacobian:', i5, 17x, &
     &         '|' )
 ...
 9400 format ( 9x, '| Integration for t > t_End enabled', 21x, '|', /, &
     &         9x, '|', 4x, 'upper t-limit: ', d12.5, 24x, '|' )
 9410 format ( 9x, '| Integration for t > t_End enabled', 21x, '|', /, &
     &         9x, '|', 4x, 'no upper t-limit', 35x, '|' )
 ...
 9460 format ( 9x, '*** Warning: This LIMEX call was specified as a', &
     &         7x, '***', /, 9x, '*** continuation call, but there ', &
     &         'was no call before.  ***', / )
 9470 format ( '   Initial solution values at t = ', d18.9, /, &
     &         ( 1x, 4d18.9 ) )
 9480 format ( ' t = ', 1pE22.13E3, /, ( 1x, 4(1pE22.13E3) ) )
 9490 format ( /, 3x, 'Step-Nr.', 5x, 'F-calls', 9x, 't', 12x, &
     &         'Stepsize', 5x, 'used/aimed stage', /, 1x, 73('-'), / )
 ...
 9520 format ( /, ' Warning: component', i5, ' does not have an ', &
     &         'asymptotic', /, 9x, ' expansion in the first ', &
     &         'integration step', / )
 ...
 9700 format ( /, ' *** LIMEX error: more than ', i4, ' stepsize ', &
     &         'reductions due to', 8x, '***', /, ' *** singular ', &
     &         'matrix, system is not solvable with LIMEX', 12x, '***' )
